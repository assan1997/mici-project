...
const ComboboxCheck: React.FC<FormikSelectFieldProps> = (
  ...
  function controlDataSelection(
    ...
    else if (selectedElementInDropdown?.find((x: {
      value: string;
      label: string;
    }) => x?.value === one?.value)) {
      setSelectedElementInDropdown((el: {
        value: string;
        label: string;
      }[] | []) => el?.filter((y: {
        value: string;
  ...
  function removeLevel(id: string) {
    ...
    else if (setSelectedUniqElementInDropdown) setSelectedUniqElementInDropdown(data);
  }
  ...
  const handleOutsideClick = useCallback((event: { target: any }) => {
    if (selectDropdownBox?.current && !selectDropdownBox?.current?.contains(event.target)) {
      setSelectDropdown("")
    }
  }, [])
  useEffect(() => {
