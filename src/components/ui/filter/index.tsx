...
export const Filter: FC<{
  ...
  dataHandler,
    ...
    const [list, setList] = useState(entryList.map((l) => ({ ...l, active: false })));
      ...
      function deepSearch(indexs: any[], level: string, cb: Function) {
        for (let element of indexs) {
          ...
          if (typeof element === "object") {
            ...
            cb(indexs.filter((item) => item && item).map((item) => item && item.toString()))
          }
        }
      }
    ...
    useEffect(() => {
      const handleSelectedDate = () => {
        ...
        const startedDay = selectedDate?.from?.day;
        const startedMonth = selectedDate?.from?.month;
        const startedYear = selectedDate?.from?.year;
        const endedDay = selectedDate?.to?.day;
        const endedMonth = selectedDate?.to?.month;
        const endedYear = selectedDate?.to?.year;
          ...
          const newDatas = filterDatas.filter((dt) => {
            const timestamp = new Date(
              `${new Date(
                ...
                dt.created_at || dt.updated_at
              ).getMonth() + 1}-${new Date(dt.created_at || dt.updated_at).getDate()}`
            ).getTime();
            const found = selectedStartedTimestamp === timestamp ||
              (selectedStartedTimestamp <= timestamp &&
                selectedEndedTimestamp >= timestamp) || selectedEndedTimestamp === timestamp;
            return found && dt;
          });
          console.log("tmp", tmp);
          console.log("newDatas", newDatas);
          ...
          <div ref={box}>
            ...
            >
              <div className="bg-white w-auto shadow-large h-auto border border-[#FFF] rounded-[12px] overlow-hidden relative">
                ...
                <Calendar
                  onChange={setSlectedDate as any}
                ...
                {list.map(({ id, name, active }) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setList(tmp => tmp.map((l) => l.id === id ? ({ ...l, active: true }) : ({ ...l, active: false })))
                    }}
                    className="flex items-center justify-start w-full cursor-pointer"
                  >
                    <div className={`text-[14px] text-justify w-full p-[4px] transition-all hover:bg-slate-50 ${active ? "bg-slate-100" : ""} text-[#000] rounded-lg font-poppins font-medium leading-[20px]`}>
                      {name}
                    </div>
                  </button>
                ))}
              ...
              <div className="bg-white w-[500px] shadow-large h-auto border border-[#FFF] rounded-[12px] overlow-hidden relative">
                <div className="w-full p-[10px]">
                  <BaseInput
                    ...
                    rightIcon={
                      processSearch ? <Spinner color={"#000"} size={20} /> : <button type="button" onClick={reset}>
                        <CloseIcon />
                      </button>}
                    type="text"
