...
export const Filter: FC<{
  type: "date" | "status" | "search" | "button";
  title: string;
  row: string;
  index?: string | number;
  indexs?: string[];
            ...
            buttonContent={
              <div className="rounded-[14px] w-auto bg-white px-[10px] border">
                <div
                  ...
                  className={`h-[40px] shrink-0  flex items-center gap-x-[10px] justify-between`}
                >
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    filterHandler((tmp: any) => {
                      if (indexs) {
                        if (level === 1) {
                          newDatas = filterDatas.filter((dt) => {
                            return dt[indexs[0] as any][indexs[1]] === id;
                          });
                        } else if (level === 2)
                          newDatas = filterDatas.filter((dt) => {
                            return (
                              dt[indexs[0] as any][indexs[1]][indexs[2]] === id
                            );
                          });
                      } else {
                        newDatas = filterDatas.filter(
