...
export const Pagination: FC<{
  ...
  const firstPage = useMemo(() => pagination.currentEnd === 10, [pagination]);
  ...
  const updatePagination = (newEnd: number, newPage: number) => {
    const slicedDatas = datas?.slice(newEnd - 10, newEnd);
    listHandler(slicedDatas);
    setPagination({ currentEnd: newEnd, currentPage: newPage });
  };

  const next = () => {
    const newEnd = pagination.currentEnd + 10;
    updatePagination(newEnd, pagination.currentPage + 1);
  };
