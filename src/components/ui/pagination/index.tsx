...
export const Pagination: FC<{
    ...
    const lastPage = useMemo(() => totalPages === currentPage, [currentPage, totalPages]);
    const next = () => {
    ...
    return <div className="w-full bg-white/80 flex justify-end items-center px-[20px] rounded-b-xl h-[60px]">
        <div className="flex gap-x-[4px]">
            ...
            <span className="text-[12px] text-[#636363] text-grayscale-900 font-medium font-poppins leading-[20px]">{totalPages}</span>
        </div>

        <button disabled={firstPage} onClick={() => {
            prev();
        }}>
            <ChevronRight size={20} color={firstPage ? "#636363" : "#000"} />
        </button>
        <button disabled={lastPage} onClick={() => {
            next();
