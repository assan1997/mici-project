...
const SideBar: FC<{ resize: boolean }> = ({ resize }) => {
  ...
  return (
    <div
      className={`h-full justify-between ${resize ? "w-[108px]" : "w-[260px]"
        }  bg-[#060606] transition-all duration-300 overflow-scroll`}
    >
      <div
        className={`w-full flex  bg-inherit ${resize ? "justify-center" : "justify-start px-[30px]"
        ...
        {nav.map(({ active, slug, id, link, icon }, index) => (
          <div className="" key={index}>
            <button
              ...
              className={` ${active ? "text-white" : "bg-inherit text-[#EBDFD7]/90"
                } shrink-0 transition-all duration-100 ${resize ? "w-[48px] justify-center" : "justify-start w-[184px]"
            ...
            >
              ...
              {!resize ? (
                <span
                  className={`${resize ? "invisible absolute" : "visible"
            ...
            <div
              ...
              className={`relative overflow-hidden flex flex-col items-start ${resize ? "pl-[4px]" : "pl-[15px]"
              ...
              <div className="py-[6px] flex flex-col gap-y-[12px] pl-[15px] border-l border-[#EBDFD7]/10">
                {getSubnavs(id)?.map(
                  ({
                    ...
                    <button
                      className={`${subNavActive
                        ? "bg-[#E65F2B]/10 text-[#E65F2B]"
                        : "bg-inherit text-white hover:bg-[#E65F2B]/10"
                        } shrink-0 transition-all duration-300 ${resize
...
const TopBar: FC<{
  ...
  const section = useMemo(() => sections.find((sec) => sec.id === user?.role_id)
    , [user, sections])
  ...
  return (
    <div className="w-full relative h-[80px] px-[20px] border-b flex items-center justify-between border-[#00000008]">
      ...
      >
        <span
          className={`${resize ? "rotate-[180deg]" : "rotate-[0deg]"
      ...
      <div className="flex justify-between items-center w-full">
        ...
        {!user ? <ProfillSkeleton /> : <div className="w-[195px] h-[50px] bg-white border flex items-center px-[5px] gap-x-[10px] justify-between rounded-full">
...
const Content: FC<{
  ...
  return (
    <div
      className={`h-full  ${resize ? "w-[calc(100%-108px)]" : "w-[calc(100%-260px)]"
