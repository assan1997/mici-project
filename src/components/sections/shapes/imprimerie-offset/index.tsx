...
export const ImprimerieOffset: FC<{}> = ({}) => {
    ...
    () =>
      allShapes?.data
        ? allShapes?.data?.filter(
            (shape: ShapeInterface) => shape.department.id === 1
          )
        : [],
    [allShapes?.data]
  );
  ...
  const handleCombineSearch = () => {
    combineSearch?.map((item) => {
      ...
      if (item.id === "Dimension LxLxH" && item?.selectedValues.length > 0) {
        combo = (combo.length === 0 ? allOffsetShapes : combo)?.filter(
